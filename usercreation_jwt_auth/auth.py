...
async def create_user(db:db_dependency,create_user_request:CreateUserRequset):
    create_user_model = Users(username=create_user_request.username,
              hashpassword = bcrypt_context.hash(create_user_request.password)
            )
...
async def login_for_access_token(form_data:Annotated[OAuth2PasswordRequestForm,Depends()],
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail="could not available user")
...
def authenticate_user(username:str,password:str,db):
    ...
    if not user:
        return False
    if not bcrypt_context.verify(password,user.hashpassword):
        return False
    return user
